...
## When to use
Use when auditing a site's off-site discoverability foundation to detect whether AI assistant crawlers (e.g. `GPTBot`, `ClaudeBot`, `PerplexityBot`) are allowed by `robots.txt`, XML sitemaps, `/llms.txt`, and static HTML rendering.
...
3. **Robots.txt Analysis**: Check `/robots.txt` for disallow rules blocking AI crawlers from `/`. Emit `critical` severity finding with exact line numbers if blocked.
4. **Sitemap & llms.txt Verification**: Check for `/sitemap.xml` and `/llms.txt`. Emit proactive suggestions if missing.
5. **JS Render Delta Analysis**: Calculate static HTML word count across `<p>`, `<h>`, and `<article>` elements. Flag `high` severity finding if raw body text is < 40 words and dynamic SPA framework markers (`id="root"`, `id="__next"`, `__NEXT_DATA__`) or large script payloads (> 10,000 chars) are detected.
